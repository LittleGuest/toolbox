use gpui::*;
use gpui_component::{sidebar::*, scroll::ScrollableElement, *};
mod views;
use views::*;
        ...
        div()
            ...
            .child(
                Sidebar::new(Side::Left)
                    ...
                    .max_w(px(280.0))
                    .children([
                        SidebarGroup::new("导航").children([
                            SidebarMenu::new().children([
                                SidebarMenuItem::new("首页").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::Home, cx);
                                })),
                                SidebarMenuItem::new("系统监控").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::SystemMonitor, cx);
                                })),
                                SidebarMenuItem::new("代码片段").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::CodeSnippet, cx);
                                })),
                                SidebarMenuItem::new("待办事项").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::Todo, cx);
                                })),
                            ]),
                        ]),
                        SidebarGroup::new("转换").children([
                            SidebarMenu::new().children([
                                SidebarMenuItem::new("文件格式转换").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::TransformFiletype, cx);
                                })),
                                SidebarMenuItem::new("时间戳").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::TransformTime, cx);
                                })),
                                SidebarMenuItem::new("进制转换").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::TransformBaseConversion, cx);
                                })),
                                SidebarMenuItem::new("OpenApi").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::TransformOpenApi, cx);
                                })),
                            ]),
                        ]),
                        SidebarGroup::new("编码/解码").children([
                            SidebarMenu::new().children([
                                SidebarMenuItem::new("Base64").on_click(cx.listener(|this, _, _, cx| {
                                ...
                                SidebarMenuItem::new("字符编码").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::EncodeDecodeCharset, cx);
                                })),
                                SidebarMenuItem::new("乱码恢复").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::EncodeDecodeMessyCode, cx);
                                })),
                            ]),
                        ]),
                        SidebarGroup::new("格式化").children([
                            SidebarMenu::new().children([
                                SidebarMenuItem::new("JSON Editor").on_click(cx.listener(|this, _, _, cx| {
                        ...
                        SidebarGroup::new("生成器").children([
                            SidebarMenu::new().children([
                                SidebarMenuItem::new("UUID").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::GeneratorUuid, cx);
                                })),
                                SidebarMenuItem::new("文本Hash").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::GeneratorHash, cx);
                                })),
                                SidebarMenuItem::new("文件校验").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::GeneratorChecksum, cx);
                                })),
                            ]),
                        ]),
                        SidebarGroup::new("数据库").children([
                            SidebarMenu::new().children([
                                SidebarMenuItem::new("假数据生成").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::DatabaseDatafaker, cx);
                                })),
                                SidebarMenuItem::new("数据库差异").on_click(cx.listener(|this, _, _, cx| {
                                    this.set_view(ViewType::DatabaseDiff, cx);
                                })),
                            ]),
                        ]),
                        SidebarGroup::new("文本").children([
                            SidebarMenu::new().children([
                                SidebarMenuItem::new("Markdown").on_click(cx.listener(|this, _, _, cx| {
            ...
            .child(
                div()
                    ...
                    .child(match current_view {
                        ...
                        ViewType::TransformFiletype => render_transform_filetype_view(self, cx),
                        ViewType::TransformTime => render_timestamp_converter_view(self, cx),
                        ViewType::TransformBaseConversion => render_base_converter_view(self, cx),
                        ViewType::TransformOpenApi => render_transform_openapi_view(self, cx),
                        ViewType::EncodeDecodeBase64 => render_base64_encoder_view(self, cx),
                        ViewType::EncodeDecodeUrl => render_url_encoder_view(self, cx),
                        ViewType::EncodeDecodeCharset => render_encode_decode_charset_view(self, cx),
                        ViewType::EncodeDecodeMessyCode => render_encode_decode_messy_code_view(self, cx),
                        ViewType::FormatterJson => render_json_editor_view(self, cx),
                        ViewType::FormatterSql => render_formatter_sql_view(self, cx),
                        ViewType::FormatterXml => render_formatter_xml_view(self, cx),
                        ViewType::GeneratorUuid => render_uuid_generator_view(self, cx),
                        ViewType::GeneratorHash => render_hash_calculator_view(self, cx),
                        ViewType::GeneratorChecksum => render_generator_checksum_view(cx),
                        ViewType::DatabaseDatafaker => render_database_datafaker_view(self, cx),
                        ViewType::DatabaseDiff => render_database_diff_view(self, cx),
                        ViewType::TextMarkdown => render_markdown_editor_view(self, cx),
                    })
            )
    }
}
...
fn render_todo_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.todo_list.is_none() {
        app.todo_list = Some(cx.new(|_| TodoList::new()));
    }
...
fn render_transform_filetype_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.transform_filetype.is_none() {
        app.transform_filetype = Some(cx.new(|_| TransformFiletype::new()));
    }
    
    if let Some(ref transform) = app.transform_filetype {
        div()
            .p_4()
            .child(transform.clone())
    } else {
        div()
            .p_4()
            .child("Loading...")
...
fn render_transform_openapi_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.transform_openapi.is_none() {
        app.transform_openapi = Some(cx.new(|_| TransformOpenapi::new()));
    }
...
fn render_encode_decode_charset_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.charset_encoder.is_none() {
        app.charset_encoder = Some(cx.new(|_| CharsetEncoder::new()));
    }
    
    if let Some(ref charset_enc) = app.charset_encoder {
        div()
            .p_4()
            .child(charset_enc.clone())
    } else {
        div()
            .p_4()
            .child("Loading...")
...
fn render_encode_decode_messy_code_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.messy_code_recover.is_none() {
        app.messy_code_recover = Some(cx.new(|_| MessyCodeRecover::new()));
    }
...
fn render_formatter_sql_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.sql_formatter.is_none() {
        app.sql_formatter = Some(cx.new(|_| SqlFormatter::new()));
    }
...
fn render_formatter_xml_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.xml_formatter.is_none() {
        app.xml_formatter = Some(cx.new(|_| XmlFormatter::new()));
    }
...
fn render_database_datafaker_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.fake_data_generator.is_none() {
        app.fake_data_generator = Some(cx.new(|_| FakeDataGenerator::new()));
    }
    
    if let Some(ref fake_gen) = app.fake_data_generator {
        div()
            .p_4()
            .child(fake_gen.clone())
    } else {
        div()
            .p_4()
            .child("Loading...")
...
fn render_uuid_generator_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.uuid_generator.is_none() {
        app.uuid_generator = Some(cx.new(|_| UuidGenerator::new()));
    }
    
    if let Some(ref uuid_gen) = app.uuid_generator {
        div()
            .p_4()
            .child(uuid_gen.clone())
    } else {
        div()
            .p_4()
            .child("Loading...")
...
fn render_hash_calculator_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.hash_calculator.is_none() {
        app.hash_calculator = Some(cx.new(|_| HashCalculator::new()));
    }
    
    if let Some(ref hash_calc) = app.hash_calculator {
        div()
            .p_4()
            .child(hash_calc.clone())
    } else {
        div()
            .p_4()
            .child("Loading...")
...
fn render_base64_encoder_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.base64_encoder.is_none() {
        app.base64_encoder = Some(cx.new(|_| Base64Encoder::new()));
    }
    
    if let Some(ref base64_enc) = app.base64_encoder {
        div()
            .p_4()
            .child(base64_enc.clone())
    } else {
        div()
            .p_4()
            .child("Loading...")
...
fn render_url_encoder_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.url_encoder.is_none() {
        app.url_encoder = Some(cx.new(|_| UrlEncoder::new()));
    }
...
fn render_timestamp_converter_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.timestamp_converter.is_none() {
        app.timestamp_converter = Some(cx.new(|_| TimestampConverter::new()));
    }
...
fn render_base_converter_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.base_converter.is_none() {
        app.base_converter = Some(cx.new(|_| BaseConverter::new()));
    }
    
    if let Some(ref base_conv) = app.base_converter {
        div()
            .p_4()
            .child(base_conv.clone())
    } else {
        div()
            .p_4()
            .child("Loading...")
...
fn render_json_editor_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.json_editor.is_none() {
        app.json_editor = Some(cx.new(|_| JsonEditor::new()));
    }
    
    if let Some(ref json_edit) = app.json_editor {
        div()
            .p_4()
            .child(json_edit.clone())
    } else {
        div()
            .p_4()
            .child("Loading...")
...
fn render_markdown_editor_view(app: &mut App, cx: &mut Context<App>) -> Div {
    if app.markdown_editor.is_none() {
        app.markdown_editor = Some(cx.new(|_| MarkdownEditor::new()));
    }
