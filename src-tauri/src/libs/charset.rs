...
pub fn encode_string(data: &str, charset: &str) -> Result<Vec<u8>> {
    let encoding = match charset {
        ...
        "UTF-32BE" => Encoding::for_label(b"UTF-32BE")
            .ok_or_else(|| Error::E("Unsupported encoding".to_string()))?,
    ...
    match input_type {
        "hex" => {
            if normalized_input.len() % 2 != 0 {
                return Err(Error::E("Hex string must have even length".to_string()));
            }
        ...
        "decimal" => {
            ...
            for part in parts {
                let byte = u8::from_str_radix(part, 10).map_err(|e| Error::E(e.to_string()))?;
                bytes.push(byte);
            }
            Ok(bytes)
        }
        "octal" => {
            ...
            for part in parts {
                ...
                let byte =
                    u8::from_str_radix(cleaned_part, 8).map_err(|e| Error::E(e.to_string()))?;
                bytes.push(byte);
            }
            Ok(bytes)
        }
...
pub fn format_bytes_to_string(
    ...
    match output_type {
        ...
        _ => Err(Error::E("Unsupported output type".to_string())),
    }
}
