...
// 重置属性
const reset = () => {
  form.enumValues = defaultValue.enumValues;
...
const previewApi = async (config) => {
  return await invoke("preview_enum", { config })
    .then((res) => {
      return res;
    })
    .catch((err) => {
...
<template>
  <n-form :model="form" label-placement="left" label-width="180">
    <!-- 枚举值 -->
    <n-form-item path="enumValues" label="枚举值">
      <n-dynamic-tags v-model:value="form.enumValues" />
    </n-form-item>
